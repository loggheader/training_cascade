...
def help():
    ...
    print "Please note that if dir and out receive the same arguments but no gray or resize flag is given no fileoverwriting will be done"
    sys.exit()
...
def get_args():
    ...
    while(j < l):
        ...
        j=j+2
    #added now
    clear_to_write_img = not( (dirname == outfolder) and (to_gray is None) and (resize_s is None) )        

clear_to_write_img=False
dirname=None
outfolder=None
bg=None
resize_s=None
